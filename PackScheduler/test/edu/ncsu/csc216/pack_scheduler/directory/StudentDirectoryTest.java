...
public class StudentDirectoryTest {
	...
	public void testAddStudent() {
		...
		try {
			impossible  = studentDirectory[1][0];
			fail("Should have caught an error");
		...
		try {
			sd.addStudent(FIRST_NAME, LAST_NAME, ID, EMAIL, "pw1", "pw2", MAX_CREDITS);
			fail("Should have caught an error");
		} catch( IllegalArgumentException e) {
		...
		/**
		try {
			sd.addStudent(null, null, null, null, "pw", "pw", 0);
			fail("Should have caught an error");
		} catch( IllegalArgumentException e) {
			assertEquals("Passwords do not match", e.getMessage());
		}
	...
	public void testGetStudentById() {
		StudentDirectory sd = new StudentDirectory();
		sd.loadStudentsFromFile(validTestFile);
		assertTrue(sd.getStudentById("zking").getFirstName().equals("Zahir"));
		assertFalse(sd.getStudentById("zking").getFirstName().equals("Dylan"));
		assertNull(sd.getStudentById("invalidId"));
	}
